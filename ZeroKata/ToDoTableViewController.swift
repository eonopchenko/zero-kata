...
class ToDoTableViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let client = MSClient(applicationURLString: "https://zerokata1.azurewebsites.net")
        let managedObjectContext = (UIApplication.shared.delegate as! AppDelegate).managedObjectContext!
    ...
    override func viewDidAppear(_ animated: Bool) {
        if winner != 0 {
            ...
            var str = ""
            if winner == 1 {
                str = player1Name + " - " + String(player1Score)
            }
            
            else if winner == 2 {
                str = player2Name + " - " + String(player2Score)
            }
            
            let itemToInsert = ["text": str, "complete": false, "__createdAt": Date()] as [String : Any]
